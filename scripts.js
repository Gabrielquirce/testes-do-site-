  ...
  form.addEventListener("submit", function (event) {
    ...
    const campos = [
      ...
      },
      {
        ...
        mensagemErro:
          "E-mail inválido! Digite um endereço válido, como exemplo@dominio.com.",
      },
      {
      ...
      },
      {
        ...
        mensagemErro: `Ano letivo inválido! Deve estar entre 1900 e ${
      ...
      },
      {
        ...
        mensagemErro:
          "Telefone inválido! Deve estar no formato (XX) XXXXX-XXXX ou (XX) XXXX-XXXX.",
      },
      {
        input: document.getElementById("inputMatricula"),
        validacao: (valor) => /^\d{6,}$/.test(valor.trim()),
        mensagemErro:
          "Matrícula inválida! Deve conter pelo menos 6 dígitos numéricos.",
      },
    ];
...
function showDatePicker(oldDate) {
  const newDateStr = prompt(
    `Editar data:\nData atual: ${oldDate.toLocaleDateString(
      "pt-BR"
  ...
  Object.keys(events).forEach((date) => {
    ...
    if (dateObj.getMonth() === currentDate.getMonth()) {
      events[date].forEach((event, index) => {
        ...
        eventItem.innerHTML = `
                    <span>${dateObj.toLocaleDateString(
                      "pt-BR"
                    )}: ${event}</span>
                    <div>
